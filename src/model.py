...
class CTCLayer(layers.Layer):
    ...
    def call(self, y_true, y_pred):
        ...
        # Dynamically calculate the actual unpadded label lengths for each sample
        # -1 is standard CTC padding token, so we count tokens that are NOT -1
        ...
        # Expand input_length to match the batch dimension
        input_length = input_length * tf.ones(shape=(batch_len, 1), dtype="int64")
        # label_length already has shape (batch_len, 1) from keepdims=True
